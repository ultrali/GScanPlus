...
class RootkitAnalysis:
    ...
    def get_kmsinfo(self):
        try:
            ...
            if os.path.exists('/proc/kallsyms'):
                self.kallsyms = os.popen("cat /proc/kallsyms 2>/dev/null|awk '{print $3}'").readlines()
                return
            elif os.path.exists('/proc/ksyms'):
                self.kallsyms = os.popen("cat /proc/ksyms").readlines()
            return
        ...
        for file in rootkit_info['file']:
            if os.path.exists(file):
                malice_result(self.name, rootkit_info['name'], file, '',
                              'Match to rootkit(%s) file rules: %s' % (rootkit_info['name'], file),
                              '[1]strings %s' % file, 'Risk',
                              programme='rm %s #Delete rootkit malicious file' % file)
                malice = True
                return suspicious, malice
        for dirname in rootkit_info['dir']:
            if os.path.exists(dirname):
                malice_result(self.name, rootkit_info['name'], dirname, '',
                              'Match to rootkit(%s) directory rules: %s' % (rootkit_info['name'], dirname),
                              '[1]ls -a %s' % dirname, 'Risk',
                              programme='rm -rf %s #Delete rootkit malicious file' % dirname)
                malice = True
                return suspicious, malice
            ...
            for ksyms in rootkit_info['ksyms']:
                if ksyms in kms:
                    malice_result(self.name, rootkit_info['name'], '/proc/kallsyms', '',
                                  'Match to rootkit(%s) Kernel symbol table characteristics: %s' % (
                                      rootkit_info['name'], ksyms),
                                  '[1]cat /proc/kallsyms', 'Risk')
                    malice = True
                    return suspicious, malice
        return suspicious, malice
    ...
    def check_bad_lkm(self):
        ...
        try:
            if not os.path.exists('/lib/modules/'):
                return suspicious, malice
            infos = os.popen(
                'find /lib/modules/ -name "*.so" -o -name "*.ko"  -o -name "*.ko.xz" 2>/dev/null').readlines()
            for file in infos:
                for lkm in self.LKM_BADNAMES:
                    if lkm == os.path.basename(file):
                        malice_result(self.name, 'LKM kernel module check', file, '',
                                      'Matching to file(%s) has malicious characteristics: %s' % (file, lkm),
                                      '[1]cat /proc/kallsyms', 'Risk',
                                      programme='rm %s #Delete rootkit malicious file' % file)
                        malice = True
                        return suspicious, malice
    ...
    def run(self):
        ...
        for rootkit_info in self.rootkit_rules:
            i += 1
            string_output(' [%d]%s' % (i, rootkit_info['name']))
            suspicious, malice = self.check_rootkit_rules(rootkit_info)
            result_output_tag(suspicious, malice)

        string_output(' [%d]LKM kernel modules' % (i + 1))
